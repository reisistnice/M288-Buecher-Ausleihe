    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        <div className="flex items-start justify-between mb-6">
          <div>
            <h1 className="text-lg font-semibold text-gray-900">My Loans</h1>
            <p className="text-sm text-gray-400 mt-0.5">Your currently borrowed books</p>
          </div>
          {!loading && !fetchError && (
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-xl px-4 py-2.5">
              ...
              <span className={`text-sm font-bold min-w-[1.5rem] text-center ${
                loans.length > 0 ? 'text-gray-900' : 'text-gray-400'
              }`}>
                {loans.length}
              </span>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-52 gap-3 text-center px-6">
              ...
              <div>
                <p className="text-sm font-medium text-gray-700">No active loans</p>
                <p className="text-xs text-gray-400 mt-0.5">Head to the catalog to borrow a book</p>
              </div>
              <button
                onClick={() => router.push('/catalog')}
                className="mt-1 rounded-lg bg-black px-4 py-2 text-xs font-semibold text-white hover:bg-gray-800 transition"
              >
                Browse Catalog
              </button>
            </div>
                    ...
                    <tr key={loan.id} className="hover:bg-gray-50/60 transition">
                      ...
                      <td className="px-5 py-4 text-right">
                        <button
                          ...
                          disabled={returning}
                          className="rounded-lg border border-gray-300 px-4 py-2 text-xs font-semibold text-gray-700 hover:border-gray-400 hover:bg-gray-50 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition min-w-[80px]"
                        >
